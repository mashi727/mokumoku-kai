...
#graphics_lw2 = pg.GraphicsLayoutWidget(show=True)        
#vb1 = graphics_lw2.addViewBox(row=0, col=1)


# 4valuesからもろもろの値を算出
def make_dataframe():
    df = create_df() # create_dfは、my_modulesの中で定義している。
    sma_short=5
    sma_med=25
    sma_long=60

    # 移動平均
    df['sma_short'] = df['Close'].rolling(sma_short).mean() # sma_shortの移動平均。sma_short分の窓をずらすので最初のsma_short-1 はNaNになる。
    df['sma_med'] = df['Close'].rolling(sma_med).mean()
...
def draw_graph(self, area, dt, df_calculated):
    ...
    p1.plot(x=dt,
            ...
            #name= self.fontCssLegend +'<p>SMA short</p>',
            pen=pen,
            )
    p1.plot(x=dt,
            y=df_calculated['sma_med'],
            #name= self.fontCssLegend +'<p>SMA med</p>',
            pen=pen1,
            )
    p1.plot(x=dt,
            y=df_calculated['sma_long'],
            #name= self.fontCssLegend +'<p>SMA long</p>',
            pen=pen2,
            )
    ...
    area.addDock(Dock('Main Chart', widget=p1, size=(1, 10))) ## place d1 at left edge of dock area (it will fill 
    ...
    region = pg.LinearRegionItem([dt[num_of_df-math.floor(num_of_df/10)], dt[num_of_df-1]]) # http://www.pyqtgraph.org/documentation/graphicsItems/linearregionitem.html
    region.setZValue(10)
    ...
    def updatePlot():
        ...
        p1.setYRange(miny*(1-0.008), maxy*(1+0.008))
...
FinData.symbol = 'dummy'
FinData.duration = 'dummy'
...
# グラフ用に各種データを生成
dt, df_calculated = make_dataframe()


from types import MethodType
MainWindow.draw_graph = MethodType(draw_graph, MainWindow)
self.draw_graph(area, dt, df_calculated)
